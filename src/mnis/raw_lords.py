...
from mnis.constants import COLUMNS_LORDS
from mnis.constants import COLUMNS_LORDS_MEMBERSHIPS
...
from mnis.utility import process_lords_output
...
def fetch_lords_raw() -> DataFrame:
    ...
    # Fetch raw
    lords_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="BasicDetails")
    ...
    rows = [
        {
            ...
            "lord_type": scalar(member.get("MemberFrom")),
        ...
        for member in lords_raw
    ]

    # Tidy
    lords = pl.from_dicts(
        rows,
        schema={column: pl.String for column in COLUMNS_LORDS})
    lords = convert_date_column(lords, "date_of_death")

    # Cache
    cache[CACHE_LORDS_RAW] = lords
...
def fetch_lords_memberships_raw() -> DataFrame:
    ...
    memberships_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="HouseMemberships")

    # Remove NULL
    memberships_raw = [
        member for member in memberships_raw
        if member.get("HouseMemberships") is not None
    ]
    ...
    for member in memberships_raw:
        mnis_id = member["@Member_Id"]
        entries = member["HouseMemberships"]["HouseMembership"]
        if isinstance(entries, dict):
            entries = [entries]
        for entry in entries:
            rows.append({
                "house_name": scalar(entry.get("House")),
                "seat_incumbency_start_date": scalar(entry.get("StartDate")),
                "seat_incumbency_end_date": scalar(entry.get("EndDate")),
                "mnis_id": mnis_id,
            })

    rows = [row for row in rows if row["house_name"] == "Lords"]

    # Tidy
    memberships = pl.from_dicts(
        rows,
        schema={column: pl.String for column in COLUMNS_LORDS_MEMBERSHIPS})
    ...
    # Combine
    memberships = process_lords_output(memberships).drop("house_name")

    # Cache
    cache[CACHE_LORDS_MEMBERSHIPS_RAW] = memberships
...
def fetch_lords_party_memberships_raw() -> DataFrame:
    ...
    party_memberships_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="Parties")
    ...
    # Extract data output for each Lord
    rows = []
    ...
    # Cache memberships
    cache[CACHE_LORDS_PARTY_MEMBERSHIPS_RAW] = memberships
...
def fetch_lords_other_parliaments_raw() -> DataFrame:
    ...
    other_parliaments_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="OtherParliaments")
    ...
    # Cache
    cache[CACHE_LORDS_OTHER_PARLIAMENTS_RAW] = other_parliaments
...
def fetch_lords_contested_elections_raw() -> DataFrame:
    ...
    contested_elections_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="ElectionsContested")
    ...
    # Cache
    cache[CACHE_LORDS_CONTESTED_ELECTIONS_RAW] = contested_elections
...
def fetch_lords_government_roles_raw() -> DataFrame:
    ...
    government_roles_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="GovernmentPosts")
    ...
    # Cache
    cache[CACHE_LORDS_GOVERNMENT_ROLES_RAW] = government_roles
...
def fetch_lords_opposition_roles_raw() -> DataFrame:
    ...
    opposition_roles_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="OppositionPosts")
    ...
    # Cache
    cache[CACHE_LORDS_OPPOSITION_ROLES_RAW] = opposition_roles
...
def fetch_lords_parliamentary_roles_raw() -> DataFrame:
    ...
    parliamentary_roles_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="ParliamentaryPosts")
    ...
    # Cache
    cache[CACHE_LORDS_PARLIAMENTARY_ROLES_RAW] = parliamentary_roles
...
def fetch_lords_maiden_speeches_raw() -> DataFrame:
    ...
    maiden_speeches_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="MaidenSpeeches")
    ...
    maiden_speeches = maiden_speeches.filter(
        pl.col("maiden_speech_house") == "Lords")

    # Combine
    maiden_speeches = process_lords_output(maiden_speeches)

    # Cache
    cache[CACHE_LORDS_MAIDEN_SPEECHES_RAW] = maiden_speeches
...
def fetch_lords_addresses_raw() -> DataFrame:
    ...
    addresses_raw = utility.fetch_query_data(
        house=HOUSE_LORDS, data_output="Addresses")
    ...
    # Cache
    cache[CACHE_LORDS_ADDRESSES_RAW] = addresses
